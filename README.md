...
│                                                                                    │
│  Task 1: PDF chính sách        Task 2: Crawl4AI news                              │
│  (tuition/scholarship/         (5 bài viết RMIT)                                  │
│   academic-calendar)                                                              │
│         │                              │                                          │
│         ▼                              ▼                                          │
...
│                         │                                                         │
│                         ▼                                                         │
│                   chroma_db/ (ChromaDB, cosine similarity)                        │
└────────────────────────┬───────────────────────────────────────────────────────┘
                          │
┌─────────────────────────┼──────────────── ONLINE — QUERY PIPELINE ───────────────┐
│                          │                                                        │
│  React (Vite) Client ──▶ POST /api/query ──▶ FastAPI (server.py, :8001)          │
│  (ChatPage, CitationList)                          │                             │
│                                                     ▼                             │
│                                    Task 10: generate_with_citation()              │
│                                    ├─ greeting? → trả lời nhanh, bỏ qua retrieval │
│                                    └─ Task 9: retrieve()                          │
│                                              │                                    │
│                    ┌─────────────────────────┼─────────────────────────┐         │
│                    ▼                                                   ▼         │
│         Task 5: semantic_search()                          Task 6: lexical_search()│
│         (dense, cosine, ChromaDB)                           (BM25, sparse)        │
│                    │                                                   │         │
│                    └─────────────────┬─────────────────────────────────┘         │
│                                       ▼                                          │
│                        Task 7: RRF merge (k=60) + rerank                         │
│                                       │                                          │
│                     best cosine score (gốc, KHÔNG phải RRF) < 0.48 ?             │
│                              │ có                    │ không                     │
│                              ▼                        ▼                          │
│              Task 8: PageIndex fallback      dùng kết quả hybrid                 │
│              (vectorless, theo cấu trúc doc)                                     │
│                              │                        │                          │
│                              └───────────┬────────────┘                          │
│                                          ▼                                       │
│                        Task 10: reorder_for_llm() (chống lost-in-the-middle)      │
│                                 format_context() → prompt                        │
│                                 OpenAI gpt-4o-mini (temperature=0.3, max_tok=1000)│
│                                          │                                       │
│                                          ▼                                       │
│                     answer + citations [Nguồn, Năm] + confidence (cosine gốc)     │
│                                          │                                       │
│                                          ▼                                       │
│                              trả về React Client hiển thị chat                   │
└────────────────────────────────────────────────────────────────────────────────┘
```

**Kiến trúc RAG (Retrieval-Augmented Generation)** — góc nhìn theo 3 khối chuẩn của mô hình RAG, mapping vào các Task cụ thể trong repo:

```
┌───────────────────────┐      ┌────────────────────────────┐      ┌───────────────────┐
│     KNOWLEDGE BASE     │      │          RETRIEVER          │      │      GENERATOR      │
│   (offline, Task 1-4)  │      │      (online, Task 5-9)     │      │   (online, Task 10) │
├───────────────────────┤      ├────────────────────────────┤      ├───────────────────┤
│ RMIT PDF + news        │      │ Dense:  semantic_search()   │      │ reorder_for_llm()   │
│  → MarkItDown → .md    │ ───▶ │  (OpenAI embedding, cosine) │ ───▶ │  (chống lost-in-    │
│  → chunk 800/100       │      │ Sparse: lexical_search()    │      │   the-middle)       │
│  → embed (1536d)       │      │  (BM25)                     │      │ format_context()    │
│  → ChromaDB            │      │ Fusion: RRF (k=60) + rerank │      │  (label nguồn)      │
│                         │      │ Fallback: PageIndex nếu     │      │ gpt-4o-mini +        │
│                         │      │  cosine gốc < 0.48          │      │  SYSTEM_PROMPT       │
│                         │      │  (vectorless, đọc theo mục  │      │  (citation, từ chối  │
│                         │      │   lục doc gốc)               │      │   câu ngoài domain)  │
└───────────────────────┘      └────────────────────────────┘      └───────────────────┘
        Task 1-4                      Task 5-9 (Role 2, 3)              Task 10 (Role 4)
                                                                    → answer + citations
```
