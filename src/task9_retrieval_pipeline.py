...
# Chốt bởi Role 1, khớp LAB_GUIDE (mục Task 9). Sau khi Task 4-5 index xong data
# thật, đo lại cosine của vài câu liên quan vs vài câu lạc đề để xác nhận 0.48 còn
# hợp lý với corpus + embedding model đang dùng (BAAI/bge-m3) — chỉnh nếu lệch.
...
def retrieve(
    ...
    # Step 4: threshold DUNG DIEM COSINE GOC (dense_results), KHONG PHAI RRF
    ...
    if best_score < score_threshold:
        ...
        try:
            ...
            print(f"  pageindex fallback failed ({exc}), giữ nguyên hybrid results")
            fallback = []
        if fallback:
