...
Phương pháp chính dùng ở đây: cross-encoder qua Jina API khi có JINA_API_KEY,
fallback về local lexical-overlap scorer (Jaccard trên token) khi không có key —
để module chạy được offline không phụ thuộc API. RRF được implement riêng và
dùng lại ở Task 9 để fuse semantic_search + lexical_search.
...
def _lexical_overlap_score(query: str, content: str) -> float:
    """Jaccard overlap giữa token query và token content. Fallback cross-encoder khi không có API key."""
...
def rerank_cross_encoder(
    ...
    scored = [
        {**c, "score": _lexical_overlap_score(query, c["content"])}
        for c in candidates
    ]
